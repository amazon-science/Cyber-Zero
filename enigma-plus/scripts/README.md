...
* `--per_instance_step_limit <int>` 💡: Per-instance step limit (interactive loop will automatically terminate when step limit is hit). Default is 40 for EnIGMA+.
* `--temperature <float>` 💡: Model temperature. Default is 0.0.
* `--top_p <float>` 💡: Top p filtering. Default is 0.95.
* `--per_instance_cost_limit <float>`: Per-instance cost limit (disabled by default for EnIGMA+, set to 0.0).
* `--total_cost_limit <float>`: Total cost limit (disabled by default for EnIGMA+, set to 0.0).
